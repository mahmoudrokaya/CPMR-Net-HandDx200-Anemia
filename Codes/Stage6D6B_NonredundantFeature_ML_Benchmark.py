...
NONREDUNDANT_FEATURE_DATASET = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D6A_NonredundantFeatureSelection\tables\nonredundant_feature_dataset.csv"
)

NONREDUNDANT_FEATURE_LIST = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D6A_NonredundantFeatureSelection\tables\selected_nonredundant_feature_names.csv"
)

OUTPUT_DIR = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D6B_NonredundantFeature_ML_Benchmark"
)
...
N_SPLITS = 5
N_REPEATS = 10
FEATURE_SET_NAME = "Nonredundant_Selected_Features"
...
if not NONREDUNDANT_FEATURE_DATASET.exists():
    raise FileNotFoundError(f"Missing file: {NONREDUNDANT_FEATURE_DATASET}")

if not NONREDUNDANT_FEATURE_LIST.exists():
    raise FileNotFoundError(f"Missing file: {NONREDUNDANT_FEATURE_LIST}")

df = pd.read_csv(NONREDUNDANT_FEATURE_DATASET)
feature_list_df = pd.read_csv(NONREDUNDANT_FEATURE_LIST)

if "feature" not in feature_list_df.columns:
    raise ValueError("selected_nonredundant_feature_names.csv must contain a 'feature' column.")

selected_features = feature_list_df["feature"].dropna().astype(str).tolist()

available_features = [f for f in selected_features if f in df.columns]
missing_features = [f for f in selected_features if f not in df.columns]

if len(available_features) == 0:
    raise ValueError("No selected nonredundant features were found in the dataset.")
...
feature_status = pd.DataFrame({
    ...
    "available_in_nonredundant_dataset": [f in available_features for f in selected_features]
})

feature_status.to_csv(
    TABLES_DIR / "selected_nonredundant_feature_availability.csv",
    index=False
)
...
report = f"""# Stage 6D6B Nonredundant Feature ML Benchmark Report

Stage 6D6B evaluated classical machine-learning models using the nonredundant feature set generated in Stage 6D6A.

## Inputs

Nonredundant feature dataset:

`{NONREDUNDANT_FEATURE_DATASET}`

Selected nonredundant feature list:

`{NONREDUNDANT_FEATURE_LIST}`
...
Selected nonredundant features requested: {len(selected_features)}

Selected nonredundant features available: {len(available_features)}
...
- selected_nonredundant_feature_availability.csv
...
This stage tests whether removing highly correlated features improves generalization, recall, F1-score, or ROC-AUC compared with the top 50 overall feature benchmark. If performance improves, redundancy pruning is beneficial. If performance remains similar or decreases, the discarded features may still contain complementary nonlinear information.
"""

with open(
    REPORTS_DIR / "Stage6D6B_NonredundantFeature_ML_Benchmark_Report.md",
    "w",
    encoding="utf-8"
) as f:
    f.write(report)
...
print("=" * 80)
print("STAGE 6D6B NONREDUNDANT FEATURE ML BENCHMARK COMPLETED")
print("=" * 80)
