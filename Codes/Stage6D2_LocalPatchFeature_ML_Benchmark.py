...
TOP_PATCH_FEATURE_FILE = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6C_Global_vs_Local_Feature_Integration_and_Filtering\tables\top50_local_patch_features.csv"
)

OUTPUT_DIR = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D2_LocalPatchFeature_ML_Benchmark"
)
...
N_SPLITS = 5
N_REPEATS = 10
FEATURE_SET_NAME = "Top50_LocalPatch_Features"
...
def get_feature_list():
    top_df = pd.read_csv(TOP_PATCH_FEATURE_FILE)

    if "feature" not in top_df.columns:
        raise ValueError("top50_local_patch_features.csv must contain a 'feature' column.")
...
if not TOP_PATCH_FEATURE_FILE.exists():
    raise FileNotFoundError(f"Missing file: {TOP_PATCH_FEATURE_FILE}")

df = pd.read_csv(PATCH_FEATURE_FILE)
...
if len(available_features) == 0:
    raise ValueError("No selected local patch features were found in participant_patch_features.csv.")
...
feature_status = pd.DataFrame({
    ...
    "available_in_patch_features": [f in available_features for f in selected_features]
})

feature_status.to_csv(
    TABLES_DIR / "selected_local_patch_feature_availability.csv",
    index=False
)
...
for model_name, model in models.items():
    ...
    # Participant-level aggregation across repeated CV predictions
    participant_pred = (
        pred_df
...
report = f"""# Stage 6D2 Local Patch Feature ML Benchmark Report

Stage 6D2 evaluated classical machine-learning models using the top 50 statistically ranked local patch features.
...
Top local patch feature ranking:

`{TOP_PATCH_FEATURE_FILE}`
...
- selected_local_patch_feature_availability.csv
...
This stage tests whether local patch-derived features provide stronger binary anemia classification performance than global whole-image features. The results should be compared directly against Stage 6D1.
"""

with open(
    REPORTS_DIR / "Stage6D2_LocalPatchFeature_ML_Benchmark_Report.md",
    "w",
    encoding="utf-8"
) as f:
    f.write(report)
...
print("=" * 80)
print("STAGE 6D2 LOCAL PATCH FEATURE ML BENCHMARK COMPLETED")
print("=" * 80)
