...
TOP_GLOBAL_FEATURE_FILE = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6C_Global_vs_Local_Feature_Integration_and_Filtering\tables\top50_global_features.csv"
)

OUTPUT_DIR = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D1_GlobalFeature_ML_Benchmark"
)
...
def get_feature_list():
    top_df = pd.read_csv(TOP_GLOBAL_FEATURE_FILE)

    if "feature" not in top_df.columns:
        raise ValueError("top50_global_features.csv must contain a 'feature' column.")
...
if not TOP_GLOBAL_FEATURE_FILE.exists():
    raise FileNotFoundError(f"Missing file: {TOP_GLOBAL_FEATURE_FILE}")

df = pd.read_csv(GLOBAL_FEATURE_FILE)
...
if len(available_features) == 0:
    raise ValueError("No selected global features were found in participant_global_features.csv.")
...
feature_status = pd.DataFrame({
    ...
    "available_in_global_features": [f in available_features for f in selected_features]
})

feature_status.to_csv(
    TABLES_DIR / "selected_global_feature_availability.csv",
    index=False
)
...
# ============================================================
# CROSS-VALIDATION EVALUATION - FIXED FOR REPEATED CV
# ============================================================
...
for model_name, model in models.items():
    ...
    # Majority / mean-probability participant-level aggregation
    participant_pred = (
        pred_df
    ...
    summary_rows.append({
        "model": model_name,
        "feature_set": "Top50_Global_Features",
...
fold_results_all = pd.DataFrame(fold_rows)
...
best_pred = prediction_tables[best_model_name]
...
plot_roc(
    best_pred["true_label"],
    best_pred["predicted_probability_anemia"],
...
report = f"""# Stage 6D1 Global Feature ML Benchmark Report

Stage 6D1 evaluated classical machine-learning models using the top 50 statistically ranked global image features.
...
Top global feature ranking:

`{TOP_GLOBAL_FEATURE_FILE}`
...
Selected features available: {len(available_features)}
...
- selected_global_feature_availability.csv
...
This stage tests whether the strongest global handcrafted RGB and thermal features can support binary anemia classification before moving to local patch features or deep learning. If performance remains low, later stages should investigate local patch features, multiscale representations, and participant-level multimodal fusion.
"""

with open(
    REPORTS_DIR / "Stage6D1_GlobalFeature_ML_Benchmark_Report.md",
    "w",
    encoding="utf-8"
) as f:
    f.write(report)
...
print("=" * 80)
print("STAGE 6D1 GLOBAL FEATURE ML BENCHMARK COMPLETED")
print("=" * 80)
