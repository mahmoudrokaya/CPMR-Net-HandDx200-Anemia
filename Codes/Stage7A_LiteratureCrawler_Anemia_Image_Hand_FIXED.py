...
def remove_excel_illegal_chars(value):
    """
    Remove characters that are valid in PDF text but illegal in Excel/docx XML output.
    This prevents openpyxl IllegalCharacterError when saving extracted abstracts/conclusions.
    """
    if value is None:
        return value
...
def clean_dataframe_for_excel(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean all object/string columns before Excel export.
    """
    cleaned = df.copy()
    for col in cleaned.columns:
        if cleaned[col].dtype == "object":
            cleaned[col] = cleaned[col].map(remove_excel_illegal_chars)
    return cleaned
...
def add_normal_paragraph(doc: Document, text: str):
    text = remove_excel_illegal_chars(text) or ""
    p = doc.add_paragraph()
    run = p.add_run(text)
    run.font.size = Pt(10)
    return p
...
def add_field(doc: Document, label: str, value: str):
    value = remove_excel_illegal_chars(value) if value else ""
    p = doc.add_paragraph()
...
def main():
    ...
    # The second condition prevents unrelated medical-imaging papers, such as malaria blood-smear papers,
    # from entering the evidence base unless anemia/hemoglobin terms are actually present.
    def is_problem_relevant(r):
        combined = f"{r.title} {r.abstract} {r.source_query}".lower()
        has_anemia_core = any(t in combined for t in [
            "anemia", "anaemia", "hemoglobin", "haemoglobin", "hb ", " hb", "pallor"
        ])
        has_image_core = any(t in combined for t in [
            "image", "imaging", "photograph", "smartphone", "camera", "vision",
            "rgb", "thermal", "infrared", "conjunctiva", "palmar", "palm",
            "hand", "finger", "fingernail", "nailbed"
        ])
        return r.relevance_score >= 8 and has_anemia_core and has_image_core

    records = [r for r in records if is_problem_relevant(r)]
    records = sorted(
        records,
        key=lambda x: (
            x.relevance_score,
    ...
    for r in records:
        r.title = remove_excel_illegal_chars(r.title)
        r.abstract = remove_excel_illegal_chars(r.abstract)
        r.authors = remove_excel_illegal_chars(r.authors)
        r.journal = remove_excel_illegal_chars(r.journal)
        r.conclusion = remove_excel_illegal_chars(r.conclusion)
        r.citation = remove_excel_illegal_chars(r.citation)
        r.notes = remove_excel_illegal_chars(r.notes)
    ...
    # Excel has a strict 32,767-character cell limit and rejects hidden control characters.
    excel_df = clean_dataframe_for_excel(df)
