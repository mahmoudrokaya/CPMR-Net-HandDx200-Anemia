...
OUTPUT_DIR = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D5_OverallFeature_ML_Benchmark"
)
...
N_SPLITS = 5
N_REPEATS = 10
FEATURE_SET_NAME = "Top50_Overall_Features"
...
def get_feature_list():
    top_df = pd.read_csv(TOP_OVERALL_FEATURE_FILE)

    if "feature" not in top_df.columns:
        raise ValueError("top50_features_overall.csv must contain a 'feature' column.")
...
for f in [GLOBAL_FEATURE_FILE, PATCH_FEATURE_FILE, TOP_OVERALL_FEATURE_FILE]:
    if not f.exists():
...
if len(available_features) == 0:
    raise ValueError("No selected overall features were found in the merged global + patch feature matrix.")
...
feature_status.to_csv(
    TABLES_DIR / "selected_overall_feature_availability.csv",
    index=False
)
...
df[
    ...
    TABLES_DIR / "overall_feature_dataset.csv",
    index=False
)
...
report = f"""# Stage 6D5 Overall Feature ML Benchmark Report

Stage 6D5 evaluated classical machine-learning models using the top 50 overall features selected from the integrated Stage 6C ranking. These features may originate from global descriptors, local patch descriptors, RGB features, or thermal features.
...
Top overall feature ranking:

`{TOP_OVERALL_FEATURE_FILE}`
...
Selected overall features available: {len(available_features)}
...
- selected_overall_feature_availability.csv
...
This stage tests the strongest overall handcrafted features regardless of their source. It serves as the main classical machine-learning benchmark before moving to nonredundant feature selection, multimodal fusion, or deep learning.
"""

with open(
    REPORTS_DIR / "Stage6D5_OverallFeature_ML_Benchmark_Report.md",
    "w",
    encoding="utf-8"
) as f:
    f.write(report)
...
print("=" * 80)
print("STAGE 6D5 OVERALL FEATURE ML BENCHMARK COMPLETED")
print("=" * 80)
