# -*- coding: utf-8 -*-
"""
Stage 10Y - Participant-Level Embedding Consistency Regularization

Builds on Stage 10X:
- Loads Stage 10V contrastive-pretrained encoders.
- Freezes encoders for warm-up.
- Progressively unfreezes encoders.
- Trains with weighted BCE for anemia classification.
- Adds within-participant view-consistency regularization.

No auxiliary modality/view/representation classification losses are used.
"""
...
STAGE_OUT = OUTPUTS_DIR / "Stage10Y_Participant_Embedding_Consistency_Regularization"
...
CONSISTENCY_WEIGHT_RGB = 0.05
CONSISTENCY_WEIGHT_THERMAL = 0.03
...
def consistency_loss_from_stack(x):
    """
    x shape: [B, V, D]
    Penalizes dispersion across view embeddings from the same participant.
    """
    center = x.mean(dim=1, keepdim=True)
    return ((x - center) ** 2).mean()
...
class ConsistencyRegularizedCPMRNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        return {
            ...
            "rgb_view_stack": rgb_stack,
            "thermal_view_stack": thermal_stack,
...
def run_epoch(model, loader, optimizer, criterion, device, train=True):
    ...
    total_loss = 0.0
    total_main_loss = 0.0
    total_rgb_consistency = 0.0
    total_thermal_consistency = 0.0
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            ...
            main_loss = criterion(out["logits"], batch["label"])
            rgb_consistency = consistency_loss_from_stack(out["rgb_view_stack"])
            thermal_consistency = consistency_loss_from_stack(out["thermal_view_stack"])

            loss = (
                main_loss
                + CONSISTENCY_WEIGHT_RGB * rgb_consistency
                + CONSISTENCY_WEIGHT_THERMAL * thermal_consistency
            )
        ...
        total_rgb_consistency += float(rgb_consistency.item()) * bs
        total_thermal_consistency += float(thermal_consistency.item()) * bs
    ...
    return {
        ...
        "rgb_consistency_loss": total_rgb_consistency / n,
        "thermal_consistency_loss": total_thermal_consistency / n,
        "metrics": metrics,
        "ids": ids_all,
        "labels": labels_all,
        "probs": probs_all,
...
def main():
    ...
    model = ConsistencyRegularizedCPMRNet(config).to(device)
    ...
    parameter_summary = {
        ...
        "consistency_weight_rgb": CONSISTENCY_WEIGHT_RGB,
        "consistency_weight_thermal": CONSISTENCY_WEIGHT_THERMAL,
    }
    ...
    for epoch in range(1, max_epochs + 1):
        if epoch == FREEZE_ENCODERS_FIRST_EPOCHS + 1:
            ...
            print("Encoders unfrozen: progressive fine-tuning with consistency regularization started.")
        ...
        row = {
            ...
            "train_rgb_consistency_loss": train_out["rgb_consistency_loss"],
            "train_thermal_consistency_loss": train_out["thermal_consistency_loss"],

            "val_loss": val_out["loss"],
            "val_main_loss": val_out["main_loss"],
            "val_rgb_consistency_loss": val_out["rgb_consistency_loss"],
            "val_thermal_consistency_loss": val_out["thermal_consistency_loss"],
        }
        ...
        if improved:
            ...
            torch.save(best_state, MODELS_OUT / "ConsistencyRegularized_CPMRNet_best_val_auc.pt")
        else:
            patience_counter += 1

        torch.save(model.state_dict(), MODELS_OUT / "ConsistencyRegularized_CPMRNet_last.pt")
        ...
        print(
            ...
            f"rgb_cons={val_out['rgb_consistency_loss']:.5f} | "
            f"th_cons={val_out['thermal_consistency_loss']:.5f} | "
    ...
    history_df.to_csv(TABLES_OUT / "consistency_regularized_training_history.csv", index=False)

    if best_val_predictions is not None:
        best_val_predictions.to_csv(PRED_OUT / "consistency_regularized_best_validation_predictions.csv", index=False)

    parameter_summary["trainable_after_unfreeze"] = int(count_parameters(model, True))
    pd.DataFrame([parameter_summary]).to_csv(
        TABLES_OUT / "consistency_regularized_parameter_summary.csv",
        index=False
    )

    summary = {
        "stage": "Stage10Y",
        "title": "Participant-Level Embedding Consistency Regularization",
        ...
        "rgb_consistency_weight": CONSISTENCY_WEIGHT_RGB,
        "thermal_consistency_weight": CONSISTENCY_WEIGHT_THERMAL,
        ...
        "best_model_file": str(MODELS_OUT / "ConsistencyRegularized_CPMRNet_best_val_auc.pt"),
        "last_model_file": str(MODELS_OUT / "ConsistencyRegularized_CPMRNet_last.pt"),
        "outputs_saved_to": str(STAGE_OUT)
    }

    with open(STAGE_OUT / "Stage10Y_Participant_Embedding_Consistency_Regularization_Summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=4, ensure_ascii=False)

    report = []
    report.append("# Stage 10Y Participant-Level Embedding Consistency Regularization\n")
    ...
    report.append(
        ...
        "within-participant view-consistency regularization to stabilize RGB and thermal branch embeddings.\n"
    )
    ...
    report.append(f"- RGB consistency weight: {summary['rgb_consistency_weight']}")
    report.append(f"- Thermal consistency weight: {summary['thermal_consistency_weight']}")
    ...
    report.append("- `tables/consistency_regularized_training_history.csv`")
    report.append("- `tables/consistency_regularized_parameter_summary.csv`")
    report.append("- `predictions/consistency_regularized_best_validation_predictions.csv`")
    report.append("- `models/ConsistencyRegularized_CPMRNet_best_val_auc.pt`")
    report.append("- `models/ConsistencyRegularized_CPMRNet_last.pt`")
    report.append("- `Stage10Y_Participant_Embedding_Consistency_Regularization_Summary.json`")

    with open(REPORTS_OUT / "Stage10Y_Participant_Embedding_Consistency_Regularization_Report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(report))

    print("=" * 80)
    print("STAGE 10Y PARTICIPANT-LEVEL EMBEDDING CONSISTENCY REGULARIZATION COMPLETED")
    print("=" * 80)
