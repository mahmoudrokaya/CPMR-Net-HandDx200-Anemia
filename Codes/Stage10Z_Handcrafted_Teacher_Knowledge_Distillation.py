# -*- coding: utf-8 -*-
"""
Stage 10Z - Handcrafted Teacher Knowledge Distillation

Purpose:
Use the strongest handcrafted-feature pipeline as a teacher to guide CPMR-Net.

Student:
- CPMR-Net initialized from Stage 10V contrastive-pretrained encoders.
- Progressive fine-tuning, as in Stage 10X.
- Main supervised loss: weighted BCE with true anemia labels.

Teacher:
- Handcrafted nonredundant-feature model probabilities.
- Uses participant-level teacher probabilities if available.
- Distillation loss aligns student probability with teacher probability.

Loss:
total = BCE(y_true, student_logits)
      + KD_WEIGHT * BCE(student_logits, teacher_probability)

No auxiliary modality/view/representation losses.
No participant consistency loss.
"""
...
ENCODER_MAP_FILE = (
    OUTPUTS_DIR
    / "Stage10V_SelfSupervised_Contrastive_Pretraining"
    / "models"
    / "contrastive_pretrained_encoder_checkpoint_map.json"
)

HANDCRAFTED_DIR = OUTPUTS_DIR / "Stage6D6B_NonredundantFeature_ML_Benchmark"

STAGE_OUT = OUTPUTS_DIR / "Stage10Z_Handcrafted_Teacher_Knowledge_Distillation"
...
# Preferred teacher file. RBFSVM was the best handcrafted model in Stage 6D6B.
TEACHER_CANDIDATE_FILES = [
    HANDCRAFTED_DIR / "tables" / "participant_level_predictions_RBFSVM.csv",
    HANDCRAFTED_DIR / "participant_level_predictions_RBFSVM.csv",
    BASE_DIR / "participant_level_predictions_RBFSVM.csv",
]
...
def find_teacher_file():
    for p in TEACHER_CANDIDATE_FILES:
        if p.exists():
            return p
    raise FileNotFoundError(
        "Could not find participant_level_predictions_RBFSVM.csv. "
        "Please copy it into Stage6D6B_NonredundantFeature_ML_Benchmark/tables "
        "or into the base project folder."
    )


def identify_probability_column(df):
    candidates = [
        "probability",
        "probability_anemia",
    ...
    possible = [c for c in df.columns if "prob" in c.lower()]
    if possible:
        return possible[0]

    raise ValueError(
        f"Could not identify teacher probability column. Available columns: {list(df.columns)}"
    )


def identify_participant_column(df):
    candidates = ["participant_id", "Participant_ID", "id", "subject_id", "patient_id"]

    for c in candidates:
        if c in df.columns:
            return c

    possible = [c for c in df.columns if "participant" in c.lower() or "subject" in c.lower()]
    if possible:
        return possible[0]

    raise ValueError(
        f"Could not identify participant column. Available columns: {list(df.columns)}"
    )


def load_teacher_probabilities():
    teacher_file = find_teacher_file()
    df = pd.read_csv(teacher_file)

    pid_col = identify_participant_column(df)
    prob_col = identify_probability_column(df)

    df[pid_col] = df[pid_col].astype(str)

    # If repeated predictions exist, average them per participant.
    teacher_df = (
        df.groupby(pid_col)[prob_col]
        .mean()
        .reset_index()
        .rename(columns={pid_col: "participant_id", prob_col: "teacher_probability"})
    )

    teacher_df["teacher_probability"] = teacher_df["teacher_probability"].clip(1e-4, 1 - 1e-4)

    return teacher_file, teacher_df
...
class ParticipantDataset(Dataset):
    def __init__(self, participant_manifest, representation_manifest, split_df, split_name, config, teacher_df):
        ...
        self.split_df["participant_id"] = self.split_df["participant_id"].astype(str)

        teacher_df = teacher_df.copy()
        teacher_df["participant_id"] = teacher_df["participant_id"].astype(str)

        self.pm = self.pm.merge(
            teacher_df,
            on="participant_id",
            how="left"
        )

        missing_teacher = self.pm["teacher_probability"].isna().sum()
        if missing_teacher > 0:
            raise ValueError(f"Missing teacher probabilities for {missing_teacher} participants.")
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "teacher_probability": torch.tensor(float(row["teacher_probability"]), dtype=torch.float32),
...
def collate_fn(batch):
    ...
    out = {
        ...
        "teacher_probability": torch.stack([b["teacher_probability"] for b in batch]),
        "representations": {"rgb": {}, "thermal": {}},
    }
...
def move_batch(batch, device):
    ...
    batch["teacher_probability"] = batch["teacher_probability"].to(device)
...
class AdaptiveFusion(nn.Module):
    ...
    def forward(self, rgb_vec, thermal_vec):
        weights = torch.softmax(
            self.gate(torch.cat([rgb_vec, thermal_vec], dim=-1)),
            dim=-1
        )
...
class DistilledCPMRNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        rgb_stack = torch.stack(rgb_specs, dim=1)
        thermal_stack = torch.stack(thermal_specs, dim=1)
...
def run_epoch(model, loader, optimizer, supervised_loss_fn, kd_loss_fn, device, train=True):
    if train:
        model.train()
    else:
        model.eval()
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            ...
            supervised_loss = supervised_loss_fn(out["logits"], batch["label"])
            kd_loss = kd_loss_fn(out["logits"], batch["teacher_probability"])

            loss = supervised_loss + KD_WEIGHT * kd_loss
        ...
        total_supervised_loss += float(supervised_loss.item()) * bs
        total_kd_loss += float(kd_loss.item()) * bs

        labels_all.extend(batch["label"].detach().cpu().numpy().tolist())
        probs_all.extend(out["probabilities"].detach().cpu().numpy().tolist())
        teacher_probs_all.extend(batch["teacher_probability"].detach().cpu().numpy().tolist())
    ...
    return {
        "loss": total_loss / n,
        "supervised_loss": total_supervised_loss / n,
        "distillation_loss": total_kd_loss / n,
        "metrics": metrics,
        "ids": ids_all,
        "labels": labels_all,
        "probs": probs_all,
        "teacher_probs": teacher_probs_all,
    }
...
def main():
    ...
    teacher_file, teacher_df = load_teacher_probabilities()
    ...
    train_ds = ParticipantDataset(pm, rm, split, "train", config, teacher_df)
    val_ds = ParticipantDataset(pm, rm, split, "val", config, teacher_df)
    ...
    model = DistilledCPMRNet(config).to(device)
    ...
    supervised_loss_fn = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    kd_loss_fn = nn.BCEWithLogitsLoss()
    ...
    teacher_df.to_csv(TABLES_OUT / "teacher_probability_table.csv", index=False)
    ...
    for epoch in range(1, max_epochs + 1):
        if epoch == FREEZE_ENCODERS_FIRST_EPOCHS + 1:
            ...
            print("Encoders unfrozen: progressive fine-tuning with teacher distillation started.")
        ...
        row = {
            ...
            "train_supervised_loss": train_out["supervised_loss"],
            "train_distillation_loss": train_out["distillation_loss"],

            "val_loss": val_out["loss"],
            "val_supervised_loss": val_out["supervised_loss"],
            "val_distillation_loss": val_out["distillation_loss"],
        }
        ...
        if improved:
            ...
            best_val_predictions = pd.DataFrame({
                ...
                "teacher_probability": val_out["teacher_probs"],
                "student_probability": val_out["probs"],
                "prediction_0_5": (np.asarray(val_out["probs"]) >= 0.5).astype(int),
            })

            torch.save(best_state, MODELS_OUT / "Distilled_CPMRNet_best_val_auc.pt")
        else:
            patience_counter += 1

        torch.save(model.state_dict(), MODELS_OUT / "Distilled_CPMRNet_last.pt")
        ...
        print(
            ...
            f"kd={val_out['distillation_loss']:.4f} | "
    ...
    history_df.to_csv(TABLES_OUT / "distilled_training_history.csv", index=False)

    if best_val_predictions is not None:
        best_val_predictions.to_csv(PRED_OUT / "distilled_best_validation_predictions.csv", index=False)

    parameter_summary["trainable_after_unfreeze"] = int(count_parameters(model, True))
    pd.DataFrame([parameter_summary]).to_csv(
        TABLES_OUT / "distilled_parameter_summary.csv",
        index=False
    )

    summary = {
        "stage": "Stage10Z",
        "title": "Handcrafted Teacher Knowledge Distillation",
        ...
        "teacher_source": str(teacher_file),
        "teacher_model": "Stage6D6B RBFSVM participant-level predictions",
        ...
        "best_model_file": str(MODELS_OUT / "Distilled_CPMRNet_best_val_auc.pt"),
        "last_model_file": str(MODELS_OUT / "Distilled_CPMRNet_last.pt"),
        "outputs_saved_to": str(STAGE_OUT)
    }

    with open(STAGE_OUT / "Stage10Z_Handcrafted_Teacher_Knowledge_Distillation_Summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=4, ensure_ascii=False)

    report = []
    report.append("# Stage 10Z Handcrafted Teacher Knowledge Distillation\n")
    report.append(f"Generated at: {summary['created_at']}\n")
    report.append("## Purpose\n")
    report.append(
        "This stage trains CPMR-Net using Stage 10V contrastive-pretrained encoders and adds "
        "knowledge distillation from the handcrafted nonredundant-feature RBFSVM teacher model.\n"
    )
    ...
    report.append(f"- Teacher source: `{summary['teacher_source']}`")
    report.append(f"- Distillation weight: {summary['distillation_weight']}")
    ...
    report.append("- `tables/teacher_probability_table.csv`")
    report.append("- `tables/distilled_training_history.csv`")
    report.append("- `tables/distilled_parameter_summary.csv`")
    report.append("- `predictions/distilled_best_validation_predictions.csv`")
    report.append("- `models/Distilled_CPMRNet_best_val_auc.pt`")
    report.append("- `models/Distilled_CPMRNet_last.pt`")
    report.append("- `Stage10Z_Handcrafted_Teacher_Knowledge_Distillation_Summary.json`")

    with open(REPORTS_OUT / "Stage10Z_Handcrafted_Teacher_Knowledge_Distillation_Report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(report))

    print("=" * 80)
    print("STAGE 10Z HANDCRAFTED TEACHER KNOWLEDGE DISTILLATION COMPLETED")
    print("=" * 80)
