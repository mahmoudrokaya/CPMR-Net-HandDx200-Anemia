# -*- coding: utf-8 -*-
"""
Stage 10S - RGB-Only Stabilized CPMR-Net

Purpose:
Train an RGB-only CPMR-Net variant to test whether the thermal branch is adding noise.
...
- LAB
- RGB texture
- RGB anatomical patches
- representation attention
- RGB view attention
- binary classifier

Thermal branch and RGB-thermal fusion are removed.
"""
...
STAGE_OUT = OUTPUTS_DIR / "Stage10S_RGB_Only_Stabilized_CPMRNet"
...
def read_rgb_tensor(path, representation):
...
class RGBOnlyParticipantDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for view in self.views:
            sample["representations"]["rgb"][view] = {}
            ...
            for rep in self.rgb_reps:
                row = view_df[view_df["representation"] == rep]

                if len(row) == 0:
                    raise RuntimeError(f"Missing RGB representation: {pid} {view} {rep}")

                sample["representations"]["rgb"][view][rep] = read_rgb_tensor(
                    row.iloc[0]["path"],
                    rep
                )
...
def rgb_collate_fn(batch):
    ...
    for view in first["representations"]["rgb"]:
        output["representations"]["rgb"][view] = {}

        for rep in first["representations"]["rgb"][view]:
            output["representations"]["rgb"][view][rep] = torch.stack(
                [b["representations"]["rgb"][view][rep] for b in batch],
                dim=0
            )
...
def move_batch_to_device(batch, device):
    ...
    for view in batch["representations"]["rgb"]:
        for rep in batch["representations"]["rgb"][view]:
            batch["representations"]["rgb"][view][rep] = (
                batch["representations"]["rgb"][view][rep].to(device)
            )
...
class RGBOnlyCPMRNet(nn.Module):
    ...
    def encode_representation(self, representation, tensor):
        if representation == "rgb_texture":
            return self.rgb_1ch_encoder(tensor)
        return self.rgb_3ch_encoder(tensor)

    def encode_rgb_specialist(self, batch, view):
        embeddings = []

        for rep in self.rgb_reps:
            tensor = batch["representations"]["rgb"][view][rep]
            emb = self.encode_representation(rep, tensor)
    ...
    def forward(self, batch):
        ...
        for view in self.views:
            spec, weights = self.encode_rgb_specialist(batch, view)
            rgb_specialists.append(spec)
            rep_weights[view] = weights

        rgb_stack = torch.stack(rgb_specialists, dim=1)
        rgb_branch, rgb_view_weights = self.rgb_view_attention(rgb_stack)

        logits = self.classifier(rgb_branch).squeeze(-1)
...
def main():
    ...
    train_ds = RGBOnlyParticipantDataset(
    ...
    val_ds = RGBOnlyParticipantDataset(
        participant_manifest,
        representation_manifest,
        holdout_split,
        "val",
    ...
    model = RGBOnlyCPMRNet(config).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if improved:
            ...
            torch.save(best_state, MODELS_OUT / "RGBOnly_CPMRNet_best_val_auc.pt")
        else:
            patience_counter += 1

        torch.save(model.state_dict(), MODELS_OUT / "RGBOnly_CPMRNet_last.pt")
    ...
    history_df.to_csv(TABLES_OUT / "rgb_only_training_history.csv", index=False)

    if best_val_predictions is not None:
        best_val_predictions.to_csv(PRED_OUT / "rgb_only_best_validation_predictions.csv", index=False)
    ...
    parameter_summary.to_csv(TABLES_OUT / "rgb_only_parameter_summary.csv", index=False)

    summary = {
        "stage": "Stage10S",
        "title": "RGB-Only Stabilized CPMR-Net",
        ...
        "removed_components": [
            "thermal_encoder",
            "thermal_view_attention",
            "adaptive_rgb_thermal_fusion"
        ],
        "retained_components": [
            "rgb_3channel_encoder",
            "rgb_1channel_texture_encoder",
            "representation_attention",
            "rgb_view_attention",
            "binary_classifier"
        ],
        ...
        "best_model_file": str(MODELS_OUT / "RGBOnly_CPMRNet_best_val_auc.pt"),
        "last_model_file": str(MODELS_OUT / "RGBOnly_CPMRNet_last.pt"),
        "outputs_saved_to": str(STAGE_OUT),
    }

    with open(STAGE_OUT / "Stage10S_RGB_Only_Stabilized_CPMRNet_Summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=4, ensure_ascii=False)

    report = []
    report.append("# Stage 10S RGB-Only Stabilized CPMR-Net\n")
    report.append(f"Generated at: {summary['created_at']}\n")
    report.append("## Purpose\n")
    report.append(
        "This stage trains an RGB-only CPMR-Net variant to determine whether the thermal branch "
        "contributes useful information or adds noise under the current training strategy.\n"
    )
    report.append("## Model Revision\n")
    report.append("- Removed thermal encoder.")
    report.append("- Removed thermal view attention.")
    report.append("- Removed RGB-thermal adaptive fusion.")
    report.append("- Retained RGB multi-representation learning and RGB view attention.\n")
    ...
    report.append("- `tables/rgb_only_training_history.csv`")
    report.append("- `tables/rgb_only_parameter_summary.csv`")
    report.append("- `predictions/rgb_only_best_validation_predictions.csv`")
    report.append("- `models/RGBOnly_CPMRNet_best_val_auc.pt`")
    report.append("- `models/RGBOnly_CPMRNet_last.pt`")
    report.append("- `Stage10S_RGB_Only_Stabilized_CPMRNet_Summary.json`")

    with open(REPORTS_OUT / "Stage10S_RGB_Only_Stabilized_CPMRNet_Report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(report))

    print("=" * 80)
    print("STAGE 10S RGB-ONLY STABILIZED CPMR-NET COMPLETED")
    print("=" * 80)
