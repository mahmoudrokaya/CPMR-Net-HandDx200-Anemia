...
TOP_RGB_FEATURE_FILE = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6C_Global_vs_Local_Feature_Integration_and_Filtering\tables\top50_rgb_features.csv"
)

OUTPUT_DIR = Path(
    r"D:\47\472\New-Papers\Anemia_Paper\Outputs\Stage6D3_RGBFeature_ML_Benchmark"
)
...
N_SPLITS = 5
N_REPEATS = 10
FEATURE_SET_NAME = "Top50_RGB_Features"
...
def get_feature_list():
    top_df = pd.read_csv(TOP_RGB_FEATURE_FILE)

    if "feature" not in top_df.columns:
        raise ValueError("top50_rgb_features.csv must contain a 'feature' column.")
...
def merge_global_and_patch_features(global_df, patch_df):
    """
    Merge global and local patch feature matrices using participant-level metadata.
    Suffixes prevent duplicate metadata conflicts.
    """
    key_cols = ["participant_id", "label", "class_name", "sex", "age"]

    available_keys_global = [c for c in key_cols if c in global_df.columns]
    available_keys_patch = [c for c in key_cols if c in patch_df.columns]

    merge_key = ["participant_id"]

    patch_drop_cols = [
        c for c in available_keys_patch
        if c != "participant_id"
    ]
...
if not TOP_RGB_FEATURE_FILE.exists():
    raise FileNotFoundError(f"Missing file: {TOP_RGB_FEATURE_FILE}")
...
if len(available_features) == 0:
    raise ValueError("No selected RGB features were found in the merged global + patch feature matrix.")
...
feature_status.to_csv(
    TABLES_DIR / "selected_rgb_feature_availability.csv",
    index=False
)
...
df[
    ...
    TABLES_DIR / "rgb_feature_dataset.csv",
    index=False
)
...
report = f"""# Stage 6D3 RGB Feature ML Benchmark Report

Stage 6D3 evaluated classical machine-learning models using the top 50 RGB features selected from the integrated Stage 6C ranking. These features may originate from either global RGB descriptors or local RGB patch descriptors.
...
Top RGB feature ranking:

`{TOP_RGB_FEATURE_FILE}`
...
Selected RGB features available: {len(available_features)}
...
- selected_rgb_feature_availability.csv
...
This stage tests whether restricting the model to RGB-derived features improves performance relative to mixed global and local feature sets. If RGB-only features outperform thermal or combined features, this supports the conclusion that anemia-related visual signals are primarily encoded in colorimetric and texture information rather than thermal intensity.
"""

with open(
    REPORTS_DIR / "Stage6D3_RGBFeature_ML_Benchmark_Report.md",
    "w",
    encoding="utf-8"
) as f:
    f.write(report)
...
print("=" * 80)
print("STAGE 6D3 RGB FEATURE ML BENCHMARK COMPLETED")
print("=" * 80)
